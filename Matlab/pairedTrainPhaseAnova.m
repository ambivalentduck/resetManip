...
functions={@launchangle,@efficiency, @betteradjustedhandpath, @adjustedhandpath, @projectionEfficiency, @handpath, @maxperpendicular};
    ...
    for name=1:length(names)
        ...
        for k=0:2
            ...
            if b<length(subject.block)
                ...
                m=mean(output(x),2);
                task=subject.block(b).stim+1;
                enhance=subject.block(b).treat+1;
